...
let toastCount = 0;
const listeners: Array<(toast: ToastProps & { id: string }) => void> = [];

export function toast(props: ToastProps & { title?: string; description?: string }) {
  const id = String(++toastCount);
  listeners.forEach((listener) => listener({ ...props, id } as ToastProps & { id: string }));
  return id;
}

type ToastWithMeta = ToastProps & { id: string; title?: string; description?: string };

export function Toaster() {
  const [toasts, setToasts] = React.useState<Array<ToastWithMeta>>([]);

  React.useEffect(() => {
    const listener = (toast: ToastWithMeta) => {
